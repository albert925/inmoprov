<?php
	...
	if (isset($_SESSION['adm'])) {
		...
		function nompropietario($dato,$serv)
		{
			...
			$sql_usu=$serv->query($sacarpropir) or die (mysqli_error());
			while ($Uus=$sql_usu->fetch_assoc()) {
				$nmsus=$Uus['nom_ap_us'];
			}
			return $nmsus;
		}
	...
	<section>
		...
		<article class="tblus">
			<?php
				...
				$ssql="SELECT * from inmuebles where $rgbb $rgcc $rgdd $rgee $rgff $rggg $rghh 
					order by $arrordenfil[$cca]";
				$rs=$conexion->query($ssql) or die (mysqli_error());
				$num_total_registros= $rs->num_rows;
				$total_paginas= ceil($num_total_registros / $tamno_pagina);
				$gsql="SELECT * from inmuebles where $rgbb $rgcc $rgdd $rgee $rgff $rggg $rghh 
		...
		<article id="automargen">
			...
			<?php
				...
				?>
							<a href="ind2x.php?pagina=<?php echo $i ?>"><?php echo "$i"; ?></a>
