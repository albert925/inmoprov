...
<body>
	...
	<section id="automargen">
		<h2 id="disubdos">Todos los Inmuebles</h2>
			...
			<article id="reciimb">
				<article id="filtros" class="filserv">
					...
					<select id="bsB">
						...
						<?php
							...
							while ($mf=$sql_fmn->fetch_assoc()) {
								...
								$nmFfmn=$mf['nam_nt'];
						?>
						<option value="<?php echo $idFfmn ?>"><?php echo "$nmFfmn"; ?></option>
						<?php
							}
						?>
					</select>
					<select id="bsC">
						...
						<?php
							...
							while ($bf=$sql_fbr->fetch_assoc()) {
								...
								$nmfbarr=$bf['nam_barr'];
						?>
						<option value="<?php echo $idfbar ?>"><?php echo "$nmfbarr"; ?></option>
						<?php
							}
						?>
					</select>
					<select id="bdD">
						...
						<?php
							...
							while ($tpf=$sql_ftp->fetch_assoc()) {
								...
								$nmFftp=$tpf['nam_tp'];
						?>
						<option value="<?php echo $idFftp ?>"><?php echo "$nmFftp"; ?></option>
						<?php
							}
						?>
					</select>
					<select id="bdE">
						<?php
							for ($sSb=0; $sSb <3 ; $sSb++) { 
						?>
						<option value="<?php echo $sSb ?>"><?php echo $arrestado[$sSb]; ?></option>
						<?php
							}
						?>
					</select>
				...
				<ul class="grid cs-style-3">
					<?php
						...
						$ssql="SELECT * from inmuebles where estd_inm<'3' order by cod_inm desc";
						$rs=$conexion->query($ssql) or die (mysqli_error());
						$num_total_registros= $rs->num_rows;
						$total_paginas= ceil($num_total_registros / $tamno_pagina);
						$gsql="SELECT * from inmuebles where estd_inm<'3' order by cod_inm desc limit $inicio, $tamno_pagina";
